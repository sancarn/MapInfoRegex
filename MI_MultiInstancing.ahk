...
GetMapInfo(){
	WinGet, LastHWND, ID, ahk_exe MapInfoPro.exe
	
	;Loop over all MapInfo COM objects
	for name, MI in GetActiveObjects("!MapInfo.Application"){
		;If MapInfo HWND == LastHWND then return MI
		if(MI.Eval("SystemInfo(9)") = LastHWND){
			return MI
		}
	}
